...
На сервере уже выполнено 20 000 обновлений на 21 обучающем материале;
проверены 10 переходов отложенного `phi30_c500`. Получено 0.285 мм RMSE
положения через 0.2 с, но 50.7 с вычисления на кадр. Подробности — в
`RESULTS.md`. Длинная прокрутка и остальные отложенные материалы ещё
не проверены. Значения ошибок старой GNS нельзя переносить на новую модель.
...
### Почему 305 тысяч параметров не означают быстрый расчёт

...
from .data import FEATURE_NAMES, Stats, build_inputs, input_features, to_tensors
...
def predict_next_frame(model, stats, history, types, phi_deg, cohesion, cfg, device,
                       boundary_kinematics, batch_size=None):
    ...
    for start in range(0, len(targets), batch_size):
        ...
        inputs = build_inputs(history, types, phi_deg, cohesion, ids, cfg.neighbors,
                              tree=tree, features=features)
        batch = {key: value.to(device) for key, value in to_tensors(inputs, stats).items()}
        delta = stats.denorm("target", predict(model, batch).cpu().numpy())
...
def rollout(model, cfg, stats, run, device, n_steps=None, batch_size=None, verbose=True):
    ...
    return {"frames": frames, "t": frames * cfg.dt, "rmse": rmse, "feature_rmse": feature_rmse,
...
def main():
    ...
    parser.add_argument("--out", default=None, help="npz for errors and pressure curves (not full particle arrays)")
